...
function processSum(numberList, callback) {
  const sumValues = numberList.reduce(function(a, b) {
    return a + b;
  }, 0);

  return callback(sumValues);
}
...
function processContains(item, list, callback) {
  const maybe = list.includes(item);
...
function getRunnersByTShirtSize(runners, tShirtSize) {
  let peopleSize = runners.filter(function(shirt) {
...
function tallyUpDonations(runners) {
  return runners.reduce((acm, currentValue) => {
    return acm + currentValue.donation;
  }, 0);
}
